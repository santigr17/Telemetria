...
//Task constants and variables for network
...
void connectESP() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    //It connected
    if (hostAttempt == 0) hostAttempt = 1;//Serial.println("Fully connected");
      else hostAttempt = 1;//Serial.println("Failed to connect to host");
      }
  //else hostAttempt = 1;//Serial.println("WiFi could not connect");
  }
//Connects the MPU
void connectMPU() {
...
//Reads values from buttons
void readButtons(int& _button1Value){//, int& _button1Value) {
  //_button0Value = digitalRead(button0Pin);
  _button1Value = digitalRead(button1Pin);
}
